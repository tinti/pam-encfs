 ...
 * pam_encfs
 *
 * Copyright (C) 2004 Anders Aagaard <aagaande@gmail.com>
 * Copyright (C) 2016 Vinicius Tinti <viniciustinti@gmail.com>
 *
...
/*
   ...
   - support changing passwords (hm.. do we want to do that?  Screwing up
   mount/umount is ok, screwing up changing password is not.)
   - option to let this count as "main" authentication, ie send OK back instead
   of ignore.
   - atm I send ignore to avoid any potentional security problems.
   - also not sure if I wanna support this, as it wouldn't work every time the
...
int converse(pam_handle_t *pamh, int nargs, struct pam_message **message,
  ...
  if (retval == PAM_SUCCESS) {
    retval = conv->conv(nargs, (const struct pam_message **)message, response,
                        conv->appdata_ptr);
  }
  return retval;
}
...
int readconfig(struct passwd *pwd, pam_handle_t *pamh, const char *user,
  ...
  while (fgets(line, BUFSIZE, conffile) != NULL) {
    if (line[0] == '#')
      continue;
    parsed = sscanf(line, "%s%s%s%s%s", username, path, targetpath,
    ...
    if (parsed == 5) {
      ...
      if (strcmp("*", username) == 0 && strcmp("-", targetpath) != 0) {
      ...
      // Check if path exists, if we're "-" then we dont care, if not we give
      // error.
      if (is_dir(path)) {
        // We may fail to stat this directory (EPERM) if it's mounted because of
        // fuse's funky permission system.
        if (!is_dir(targetpath)) {
          if (checkmnt(targetpath)) {
            // Doublecheck if we're mounted, for some reason we can't stat the
            // dir even when root if it's mounted.
            // we are mounted, but we return 1 anyway so we can store targetpath
            fclose(conffile);
            return 1;
          }
          _pam_log(LOG_ERR, "TargetPath for %s does not exist (%s)", user,
                   targetpath);
          fclose(conffile);
          return 0;
        }
      ...
      // Path does not exist, if we're a specified user give error, if not keep
      // looking.
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc,
  ...
  // Read configfile
  if (!readconfig(pwd, pamh, pwd->pw_name, path, targetpath, encfs_options,
                  fuse_options)) {
    // DEBUG _pam_log(LOG_ERR,"No entry for user found in log");
    return PAM_IGNORE;
  }
  // DEBUG _pam_log(LOG_ERR,"Username : %s, Encpath : %s, Targetmount :
  // %s",pwd->pw_name,path,targetpath);

  // Store targetpath
  targetpath_store = strdup(targetpath);
  if ((i = pam_set_data(pamh, "encfs_targetpath", targetpath_store,
  ...
  /*  _pam_log(LOG_ERR,"Config output for %s:",user);
     ...
     _pam_log(LOG_ERR,"  encfs      : %s
     %s",default_encfs_options,encfs_options);
  ...
  case 0:
    ...
    if (drop_permissions == 1)
      if ((initgroups(pwd->pw_name, pwd->pw_gid) == -1) ||
          (setgid(pwd->pw_gid) == -1) || (setuid(pwd->pw_uid) == -1)) {
    ...
    // For some reason the current directory has to be set to targetpath (or
    // path?) before exec'ing encfs through gdm
    chdir(targetpath);
    execvp("encfs", arg);
    char errstr[128];
...
PAM_EXTERN int pam_sm_close_session(pam_handle_t *pamh, int flags, int argc,
  ...
  if ((retval = pam_get_data(pamh, "encfs_targetpath",
  ...
  /*We'll get this error every single time we have more than one session active,
     todo fix this with some better checks + support fuser -km if no more
     session connected.
     if (checkmnt(targetpath)) {
